class SegmentationModel:
  ...
  def segnet_model(self):
    ...
    x = layers.Conv2D(filters=128, kernel_size=(3,3), padding="same")(p1)
    x = layers.BatchNormalization()(x)
    x = layers.Activation("relu")(x)
    ...
    x = layers.Conv2D(filters=256, kernel_size=(3,3), padding="same")(p2)
    x = layers.BatchNormalization()(x)
    x = layers.Activation("relu")(x)
    ...
    x = layers.Conv2D(filters=512, kernel_size=(3,3), padding="same")(p3)
    x = layers.BatchNormalization()(x)
    x = layers.Activation("relu")(x)
    ...
    x = layers.Conv2D(filters=512, kernel_size=(3,3), padding="same")(p4)
    x = layers.BatchNormalization()(x)
    x = layers.Activation("relu")(x)
    x = layers.Conv2D(filters=512, kernel_size=(3,3), padding="same")(x)
    x = layers.BatchNormalization()(x)
    ...
    x = layers.BatchNormalization()(x)
    x = layers.Activation("relu")(x)
    # p5, m5 = MaxPoolingWithArgmax2D(pool_size=(2,2))(x)
    p5 = layers.MaxPooling2D(pool_size=(2,2))(x)

    #SegNet Decoder

    # x = MaxUnpooling2D((2,2))([p5,m5])
